...
<dialog bind:this={deleteModal} class="modal max-sm:modal-bottom">
	<div class="modal-box">
		...
		{#if selectedDataForDeletion}
			<div class="grid grid-cols-4 gap-y-2 py-4 text-sm">
				<span class="font-medium">ID</span>
				<span class="col-span-3 font-medium">{selectedDataForDeletion.id}</span>
				{#each columns as column}
					{@const key = column as never}
					<span>{titleCase(key)}</span>
					<span class="col-span-3 line-clamp-2 text-wrap break-all underline"
